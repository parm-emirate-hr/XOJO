#tag MobileContainer
Begin MobileContainer Container_Movies_Offers
   AccessibilityHint=   ""
   AccessibilityLabel=   ""
   Compatibility   =   ""
   Device = 1
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  var OfferArray() as Dictionary = offerAPI.getList
		  ...
		  for each a as Dictionary in OfferArray
		    ...
		    containers(i).theMovie.Position =0.5
		    
		    containers(i).heading.visible = true
		    containers(i).subheading.visible = true
		    containers(i).theMovie.LoadURL(mediaAPI.getUrl("offers",a.value("id"),a.value("video")))
