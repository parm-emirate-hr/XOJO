#tag MobileScreen
Begin MobileScreen challenges_Screen Implements ARMobileTableSectionHeaderSource, iOSMobileTableDataSource
   BackButtonCaption=   ""
   Compatibility   =   ""
   ControlCount    =   0
   Device = 7
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function RowCount(table As iOSMobileTable, section As Integer) As Integer
		  ...
		  #Pragma Unused table
		  return app.challengeSummary.Count
		End Function
	#tag EndMethod
		...
		Function RowData(table As iOSMobileTable, section As Integer, row As Integer) As MobileTableCellData
		  ...
		  case 0
		    if app.player <> nil then
		      Var c As MobileTableCellData=table.CreateCustomCell(GetTypeInfo(challengesCustomCell))
		      Var cc As challengesCustomCell=challengesCustomCell(c.Control)
		      cc.challenge_card1.challenge = dictionary(app.challengeSummary(row))
		      cc.challenge_card1.redraw
		      return c
		    else
