...
Protected Module articleAPI
	...
	#tag Method, Flags = &h0
		Function getList() As dictionary()
		  ...
		  var query as string = App.hostname  +"/"+ "articles"
		  ...
		  if myconnection.HTTPStatusCode = 200 then
		    ...
		    var offersArray() as variant = theresult.Value("articles")
		    var TheDataArray() as dictionary 
		    for each d as dictionary in offersArray
		      TheDataArray.Add(d)
		    next
		    app.articles = theDataArray
		    return TheDataArray
		  else
		    MessageBox("Error in articles getList ")
